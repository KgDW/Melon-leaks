package melon.system.render.graphic.texture
...
object TextureUtils {
    ...
    fun BufferedImage.getAlpha(): ByteArray {
        ...
        val data =
            when (val dataType = raster.dataBuffer.dataType) {
        ...
        for (y in 0 until height) {
            for (x in 0 until width) {
                alphaArray[index++] =
                    colorModel.getAlpha(raster.getDataElements(x, y, data)).toByte()
            }
        }
    ...
    inline fun IResource.readImage(): BufferedImage {
        return this.inputStream.use { ImageIO.read(it) }
    }
}
