...
class TextComponent(val separator: String = " ") {
    ...
    constructor(
        string: String,
        separator: String = " ",
        vararg delimiters: String = arrayOf(separator)
    ) : this(separator) {
    ...
    fun add(textElement: TextElement) {
        ...
        // Add text element to current line, and create new text line object if current line has
        // null
        textLines[currentLine] =
            (textLines[currentLine] ?: TextLine(separator)).apply { this.add(textElement) }
    }
    ...
    fun getWidth() = textLines.maxOfOrNull { it?.getWidth() ?: 0.0f } ?: 0.0f

    fun getHeight(lineSpace: Int, skipEmptyLines: Boolean = true) =
        MainFontRenderer.getHeight() * getLines(skipEmptyLines) +
            lineSpace * (getLines(skipEmptyLines) - 1)

    fun getLines(skipEmptyLines: Boolean = true) =
        textLines.count { !skipEmptyLines || (it != null && !it.isEmpty()) }
    ...
    class TextLine(private val separator: String) {
        ...
        fun drawLine(alpha: Float, horizontalAlign: HAlign) {
            ...
            for (textElement in textElementList) {
                ...
                glTranslatef(
                    MainFontRenderer.getWidth(textElement.text) +
                        MainFontRenderer.getWidth(adjustedSeparator),
                    0f,
                    0f
                )
            }
