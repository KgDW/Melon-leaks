...
import melon.system.render.font.glyph.GlyphChunk
import melon.system.render.font.renderer.AbstractFontRenderContext
import melon.system.render.font.renderer.AbstractFontRenderer
import melon.system.render.graphic.buildAttribute
import melon.system.render.graphic.shaders.DrawShader
import melon.system.render.graphic.use
import melon.system.util.io.skip
...
class RenderString(fontRenderer: AbstractFontRenderer, private val string: CharSequence) {
    ...
    fun build(
        ...
        val builderArray = Array(3) { arrayOfNulls<StringRenderInfo.Builder>(128) }
    ...
    fun render(modelView: Matrix4f, color: ColorRGB, drawShadow: Boolean, lodBias: Float) {
        ...
        renderInfoList.forEach { it.render(drawShadow, lodBias) }
    ...
    fun tryClean(current: Long): Boolean {
        return if (
            current - initTime >= 15000L ||
                invalid && current - initTime >= 1000L ||
                current - lastAccess >= 15000L
        ) {
            destroy()
            true
        } else {
    ...
    fun destroy() {
        renderInfoList.forEach { it.destroy() }
    ...
    private object Shader :
        DrawShader(
            "/assets/melon/shaders/general/FontRenderer.vsh",
            "/assets/melon/shaders/general/FontRenderer.fsh"
        ) {
        val defaultColorUniform = glGetUniformLocation(id, "defaultColor")

        init {
            use { glUniform1i(glGetUniformLocation(id, "texture"), 0) }
        }
    ...
    private constructor(
        ...
        class Builder(private val glyphChunk: GlyphChunk, private val shadowDist: Float) {
            ...
            fun put(
                ...
                posY: Float,
                charInfo: CharInfo,
                context: AbstractFontRenderContext
            ) {
            ...
            private companion object {
                val vertexAttribute =
                    buildAttribute(16) {
                        float(0, 2, melon.system.render.graphic.GLDataType.GL_FLOAT, false)
                        float(1, 2, melon.system.render.graphic.GLDataType.GL_UNSIGNED_SHORT, true)
                        int(2, 1, melon.system.render.graphic.GLDataType.GL_BYTE)
                        float(3, 1, melon.system.render.graphic.GLDataType.GL_UNSIGNED_BYTE, false)
                        float(4, 1, melon.system.render.graphic.GLDataType.GL_UNSIGNED_BYTE, false)
                    }
            }
        }
    }
