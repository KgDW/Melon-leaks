...
import melon.system.render.font.RenderString
import melon.system.render.font.Style
import melon.system.render.font.glyph.FontGlyphs
import dev.zenhao.melon.utils.TickTimer
import melon.system.render.graphic.GlStateUtils
import melon.system.render.graphic.MatrixUtils
import melon.system.util.color.ColorRGB
import melon.utils.concurrent.threads.onMainThread
import java.awt.Font

abstract class AbstractFontRenderer(font: Font, size: Float, private val textureSize: Int) :
    IFontRenderer {
    ...
    protected fun loadFont(font: Font, size: Float, style: Style): FontGlyphs {
        ...
        val fallbackFont =
            try {
                ...
                Melon.logger.warn("Failed loading fallback font. Using Sans Serif font", e)
                getSansSerifFont().deriveFont(style.styleConst, size)
            }

        return FontGlyphs(
            style.ordinal,
            font.deriveFont(style.styleConst, size),
            fallbackFont,
            textureSize
        )
    }
    ...
    override fun drawString(
        ...
        val stringCache =
            renderStringMap.computeIfAbsent(charSequence.toString()) {
        ...
        val modelView =
            MatrixUtils.loadModelViewMatrix()
                .getMatrix()
    ...
    companion object {
        fun getFallbackFont(): Font {
            return Font(
                fallbackFonts.firstOrNull { NewCustomFont.availableFonts.containsKey(it) },
                Font.PLAIN,
                64
            )
        }
