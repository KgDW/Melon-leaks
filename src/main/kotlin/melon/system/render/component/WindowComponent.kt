...
open class WindowComponent(
    ...
    width: Float,
    height: Float,
) : InteractiveComponent(name, posX, posY, width, height) {
    ...
    val renderMinimizeProgress by AutoUpdateValue {
    ...
    override fun onDrag(mousePos: Vec2f, clickPos: Vec2f, buttonId: Int) {
        ...
        val verticalSide = when (relativeClickPos.y) {
            in -2.0..centerSplitterVCenter -> TOP
            in centerSplitterVCenter..preDragSize.y - centerSplitterV -> CENTER
            in preDragSize.y - centerSplitterV..preDragSize.y + 2.0 -> BOTTOM
            else -> null
        }
        ...
        if (horizontalSide != null && verticalSide != null) {
            if (resizable && !minimized && (horizontalSide != HAlign.CENTER || verticalSide != CENTER)) {
    ...
    fun isInWindow(mousePos: Vec2f): Boolean {
        return visible && mousePos.x in preDragPos.x - 2.0f..preDragPos.x + preDragSize.x + 2.0f && mousePos.y in preDragPos.y - 2.0f..preDragPos.y + max(preDragSize.y * renderMinimizeProgress, draggableHeight) + 2.0f
    }
