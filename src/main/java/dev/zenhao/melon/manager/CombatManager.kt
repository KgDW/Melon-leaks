...
import dev.zenhao.melon.mixin.client.accessor.AccessorSPacketEntityStatus
import dev.zenhao.melon.module.modules.crystal.CrystalDamage
...
import melon.utils.combat.CrystalUtils.blockPos
...
object CombatManager : AlwaysListening {
    ...
    val SPacketEntityStatus.entityID: Int
        get() = (this as AccessorSPacketEntityStatus).melonGetEntityID()
        ...
        safeEventListener<PacketEvents.Receive>(114514) { event ->
            when (event.packet) {
                ...
                is SPacketEntityMetadata -> {
                    ...
                    val entity = world.getEntityByID(event.packet.entityId) as? EntityLivingBase? ?: return@safeEventListener
    ...
    private fun SafeClientEvent.updateCrystalDamage() {
        ...
        if (flag1 || flag2) {
            val predictPosSelf = trackerSelf?.calcPosAhead(MelonAura2.ownPredictTicks.value) ?: player.positionVector
            val contextSelf = CalcContext(this, player, predictPosSelf)

            val target = CombatManager.target
            val contextTarget = target?.let {
                val predictPos = trackerTarget?.calcPosAhead(8) ?: it.positionVector
