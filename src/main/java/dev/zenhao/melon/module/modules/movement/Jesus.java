...
import dev.zenhao.melon.event.events.entity.JesusEvent;
...
public class Jesus extends Module {
    ...
    public void onUpdate() {
        ...
        if ((this.mode.getValue() == Mode.BOUNCE || this.mode.getValue() == Mode.VANILLA || this.mode.getValue() == Mode.NORMAL) && !mc.player.isSneaking() && !mc.player.noClip && !mc.gameSettings.keyBindJump.isKeyDown() && EntityUtil.isInLiquid()) {
            mc.player.motionY = 0.10000000149011612;
        }
        if (this.mode.getValue() == Mode.TRAMPOLINE && (this.eventMode.getValue() == EventMode.ALL || this.eventMode.getValue() == EventMode.PRE)) {
            this.doTrampoline();
        } else if (this.mode.getValue() == Mode.TRAMPOLINE && (this.eventMode.getValue() == EventMode.ALL || this.eventMode.getValue() == EventMode.POST)) {
            this.doTrampoline();
        }
    }
    ...
    public void onLiquidCollision(final JesusEvent event) {
        ...
        if ((this.mode.getValue() == Mode.BOUNCE || this.mode.getValue() == Mode.VANILLA || this.mode.getValue() == Mode.NORMAL) && mc.world != null && mc.player != null && EntityUtil.checkCollide() && mc.player.motionY < 0.10000000149011612 && event.getPos().getY() < mc.player.posY - 0.05000000074505806) {
