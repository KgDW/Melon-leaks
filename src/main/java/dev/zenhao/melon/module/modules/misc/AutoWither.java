...
public class AutoWither extends Module {
    ...
    private final ModeSetting<?> useMode = msetting("Mode", UseMode.SINGLE);
    ...
    private static void placeBlock(BlockPos pos, boolean rotate) {
        ...
        if (rotate) {
            RotationManager.addRotations(hitVec);
        }
    ...
    public void onUpdate() {
        ...
        if (this.buildStage == 1) {
            ...
            List<BlockPos> blockPosList = CrystalUtil.getSphere(mc.player.getPosition().down(), this.placeRange.getValue(), this.placeRange.getValue().intValue(), false, true, 0);
