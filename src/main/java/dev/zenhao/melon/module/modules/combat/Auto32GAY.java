...
public class Auto32GAY extends Module {
    ...
    public static Auto32GAY INSTANCE = new Auto32GAY();
    ...
    private final IntegerSetting placeRange = isetting("PlaceRange", 4, 1, 6);
    ...
    public BlockPos hopperBlockPos;
    ...
    public void onEnable() {
        ...
        if (mode.getValue().equals(Mode.Auto)) {
            List<BlockPos> placeTargetList = CrystalUtil.getSphereVec(getPlayerPos(), placeRange.getValue(), placeRange.getValue(), false, true, 0);
            ...
            if (rotate.getValue()) {
                RotationManager.addRotations(BlockInteractionHelper.getLegitRotations(new Vec3d(placeTarget.x, placeTarget.y, placeTarget.z))[0], BlockInteractionHelper.getLegitRotations(new Vec3d(placeTarget.x, placeTarget.y, placeTarget.z))[1]);
            }
            if (debugMessages.getValue()) {
        ...
        mc.player.inventory.currentItem = shulkerSlot;
        hopperBlockPos = new BlockPos(placeTarget);
        mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(hopperBlockPos, EnumFacing.DOWN, EnumHand.MAIN_HAND, 0, 0, 0));
