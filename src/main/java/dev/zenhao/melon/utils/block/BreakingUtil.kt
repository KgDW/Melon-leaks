...
open class BreakingUtil {
    companion object {
        ...
        fun calcBreakTime(player: Int, pos: BlockPos): Float {
            ...
            if (p != null) {
                val breakSpeed = if (p != mc.player) getBreakSpeedObi(p, blockState) else getBreakSpeed(
                    p,
                    blockState
        ...
        fun calcBreakTime(player: EntityPlayer, pos: BlockPos): Float {
            ...
            val hardness = blockState.getBlockHardness(mc.world, pos)
            val breakSpeed =
                if (player != mc.player) getBreakSpeedObi(player, blockState) else getBreakSpeed(player, blockState)
            if (breakSpeed == -1.0f) {
                return -1f
            }
