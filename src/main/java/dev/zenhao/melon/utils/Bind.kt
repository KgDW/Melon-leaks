...
class Bind(modifierKeysIn: IntAVLTreeSet, keyIn: Int) {
    ...
    val isEmpty
        get() = key !in 1..255 && key !in -16..-1

    fun isDown(): Boolean {
        return !isEmpty &&
            if (key >= 0) Keyboard.isKeyDown(key)
            else
                Mouse.isButtonDown(-key - 1) &&
    ...
    companion object {
        private val modifierName =
            Int2ObjectLinkedOpenHashMap<String>().apply {
        ...
        private val priorityMap =
            Int2IntLinkedOpenHashMap().apply {
                val priorityKey =
                    arrayOf(
                        Keyboard.KEY_LCONTROL,
                        Keyboard.KEY_RCONTROL,
        ...
        val keyComparator = compareBy<Int> { priorityMap[it] }
    }
}
