...
import dev.zenhao.melon.gui.font.CFont;
import dev.zenhao.melon.gui.font.CFont;
    ...
    public float drawString(String text, double x, double y, float speed, float saturation, float brightness, long add, int alpha, boolean shadow) {
        ...
        if (render) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                GL11.glEnd();
                if (strikethrough) drawLine(x, y + currentData[character].height / 2f, x + currentData[character].width - 8.0D, y + currentData[character].height / 2, 1.0F);
