...
import dev.zenhao.melon.utils.block.BlockUtil.BlockResistance;
...
public class HoleUtil {
    ...
    public static BlockPos is2Hole(BlockPos pos) {
        ...
        if (size2 == 8) {
            tempX = (pos.getX() + blockpos2.getX()) / 2f;
            tempZ = (pos.getZ() + blockpos2.getZ()) / 2f;
            centerPos = new BlockPos(tempX, blockpos2.getY(), tempZ);
            return blockpos2;
        }
        return null;
    }

    public static BlockPos getCenterPos() {
        if (centerPos != null) {
            return centerPos;
        }
        return null;
